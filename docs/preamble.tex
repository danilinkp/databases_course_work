...
\DeclareCaptionFormat{listing}{\raggedright#1#2#3\par}
...
\newcommand{\appsection}{
    ...
    % Вот эта строка теперь будет автоматически делать листинги вида А.1, Б.1
    \renewcommand{\thelstlisting}{\theappsection.\arabic{lstlisting}} 
    \renewcommand{\theequation}{\theappsection.\arabic{equation}}

    \setcounter{figure}{0}
    \setcounter{table}{0}
    \setcounter{lstlisting}{0} % Сброс счетчика листингов для нового приложения
...
% \tcbuselibrary{listings, breakable, skins, fitting, hooks}

\tcbuselibrary{listings, breakable}
\newtcblisting{sqlblock}[1]{
  breakable,
  title=#1,
  title after break=\textit{#1 (продолжение)},
  listing options={
    language=SQL,
  ...
  colback=gray!5, colframe=gray!75!black, arc=2mm, boxrule=0.5pt,
  listing only,
  fonttitle=\bfseries
}
