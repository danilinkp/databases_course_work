...
\begin{lstlisting}[caption={Функция подсчёта итоговой суммы заказа. Часть 1}, label={lst:calculate_total}, language=SQL]
...
BEGIN
    ...
    WHERE ca.customer_id = v_order.customer_id AND ca.is_default = TRUE
    LIMIT 1;
\end{lstlisting}
\begin{lstlisting}[caption={Функция подсчёта итоговой суммы заказа. Часть 2}, label={lst:calculate_total_part2}, language=SQL]
    SELECT *
    INTO v_zone
    FROM delivery_zones
    WHERE restaurant_id = v_order.restaurant_id
    ...
    END IF;
\end{lstlisting}
\begin{lstlisting}[caption={Функция подсчёта итоговой суммы заказа. Часть 3}, label={lst:calculate_total_part3}, language=SQL]
    SELECT COUNT(*)
    INTO v_orders_count
    FROM orders
...
\begin{lstlisting}[caption={Триггер для автоматического обновления рейтинга ресторана}, label={lst:update_rating}, language=SQL]
...
BEGIN
    ...
    IF v_restaurant_id IS NULL THEN
        RETURN NEW;
    END IF;
    SELECT ROUND(AVG(r.restaurant_rating)::NUMERIC, 2) INTO v_new_rating
    FROM reviews r
...
\begin{lstlisting}[caption={Триггер для автоматического обновления рейтинга курьера}, label={lst:update_courier_rating}, language=SQL]
...
DECLARE
    ...
    v_new_rating  NUMERIC;
BEGIN
    SELECT o.courier_id INTO v_courier_id
    FROM orders o
    WHERE o.order_id = NEW.order_id;
    IF v_courier_id IS NULL THEN
        RETURN NEW;
    END IF;
    SELECT ROUND(AVG(r.courier_rating)::NUMERIC, 2) INTO v_new_rating
    FROM reviews r
    JOIN orders o ON o.order_id = r.order_id
